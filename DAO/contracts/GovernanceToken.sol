...
//Tokni z katerimi se bo lahko glasuje
contract GovernanceToken is ERC20Votes {
    ...
    function claimTokens() external {
        require(!s_claimedTokens[msg.sender], "Already claimed tokens"); // enak user ne sme 2x claimat
        s_claimedTokens[msg.sender] = true;
        _transfer(address(this), msg.sender, TOKENS_PER_USER * 10**18);
        s_holders.push(msg.sender); // dodaj na s_holders
    }

    function getHolderLength() external view returns (uint256) {
        return s_holders.length; // Koliko holderjev je
    }
